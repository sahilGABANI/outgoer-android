...
class NewMyPostAdapter(private val context: Context) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.PostItemViewType.ordinal -> {
                PostViewHolder(NewUserPostView(context).apply {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (adapterItem) {
            is AdapterItem.PostViewItem -> {
                (holder.itemView as NewUserPostView).bind(adapterItem.data)
            }
        }
    }
