...
import com.outgoer.ui.sponty.view.SpontyListView
import io.reactivex.Observable
import io.reactivex.subjects.PublishSubject

class HomeSpontyAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.SpontyViewType.ordinal -> {
                SpontyViewHolder(HomeSpontysView(context).apply {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (adapterItem) {
            is AdapterItem.SpontyViewItem -> {
                (holder.itemView as HomeSpontysView).bind(adapterItem.spontyResponse, userId)
            }
            else -> {}
        }
    }
