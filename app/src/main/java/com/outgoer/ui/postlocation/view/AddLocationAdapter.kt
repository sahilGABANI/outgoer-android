...
class AddLocationAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ViewType.PlacesViewType.ordinal -> {
                AddPlacesViewHolder(AddLocationView(context).apply {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (adapterItem) {
            is AdapterItem.PlacesViewItem -> {
                (holder.itemView as AddLocationView).bind(adapterItem.resultResponse)
            }
        }
    }
    ...
    private class AddPlacesViewHolder(view: View) : RecyclerView.ViewHolder(view)
