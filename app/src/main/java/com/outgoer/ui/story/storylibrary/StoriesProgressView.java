package com.outgoer.ui.story.storylibrary;
...
public class StoriesProgressView extends LinearLayout {
    ...
    public void startStories() {
        if (progressBars.size() != 0) progressBars.get(0).startProgress();
    }
    ...
    public void pauseStory(int index) {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                progressBars.get(index).pauseProgress();
            }
        }, 100);
    }

    public void resumeStory(int index) {
        if (index < progressBars.size())
            progressBars.get(index).resumeProgress();
    }

    public void setDurationStory(long duration, int index) {
        if (index < progressBars.size())
            progressBars.get(index).updateDuration(duration);
    }
}
