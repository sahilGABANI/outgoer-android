...
import com.outgoer.databinding.ActivityVenueTimingUpdateBinding
...
class VenueTimingUpdateActivity : BaseActivity() {

    private lateinit var binding: ActivityVenueTimingUpdateBinding
    ...
    companion object {
        ...
        fun getIntent(context: Context, registerVenueRequest: RegisterVenueRequest): Intent {
            var intent = Intent(context, VenueTimingUpdateActivity::class.java)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityVenueTimingUpdateBinding.inflate(layoutInflater)
    ...
    private fun listenToViewModel() {
        createVenueViewModel.groupState.subscribeAndObserveOnMainThread {
            when(it) {
                is CreateVenueViewModel.GroupViewState.UpdateVenueSuccess -> {
                    finish()
                }
                else -> {
    ...
    private fun initUI() {
        ...
        intent?.let {
            ...
            listofvenue = registerVenueRequest?.vanueAvailibility ?: listofvenue
        ...
        binding.continueMaterialButton.throttleClicks().subscribeAndObserveOnMainThread {
           // updateAvailanility()
            registerVenueRequest?.let {
                ...
                createVenueViewModel.updateVenue(it)
            }
        }
    }
