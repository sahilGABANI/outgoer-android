...
import com.outgoer.databinding.PopUpViewChatShareReceiverBinding
import com.outgoer.databinding.ViewNewChatImageReceiverBinding
import com.outgoer.databinding.ViewNewChatShareReceiverBinding
import io.reactivex.Observable
import io.reactivex.subjects.PublishSubject

class NewChatShareReceiverView(context: Context) : ConstraintLayoutWithLifecycle(context) {
    ...
    private var binding: ViewNewChatShareReceiverBinding? = null
    ...
    private fun inflateUi() {
        val view = View.inflate(context, R.layout.view_new_chat_share_receiver, this)
        layoutParams = LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT)
        binding = ViewNewChatShareReceiverBinding.bind(view)
        binding?.let {

            it.ivUserProfileImage.throttleClicks().subscribeAndObserveOnMainThread {
                imageReceiverClickSubject.onNext(chatMessageInfo)
            }
            it.ivMediaView.throttleClicks().subscribeAndObserveOnMainThread {
                shareViewClicksSubject.onNext(chatMessageInfo)
            }
    ...
    private fun showCustomPopupMenu(longPressedView: View) {
        ...
        val popupView = PopUpViewChatShareReceiverBinding.inflate(LayoutInflater.from(context), parentView, false)
        ...
        if (::chatMessageInfo.isInitialized) {
            with(popupView) {
                ...
                if (MessageType.Reel.equals(chatMessageInfo.fileType)) {
                    Glide.with(context)
                        .load(chatMessageInfo.reelObj?.thumbnailUrl)
                        .placeholder(colorDrawable)
                        .centerCrop()
                        .into(ivMediaView)

                    postTypeLinearLayout.visibility = View.GONE
                    reelTypeAppCompatImageView.visibility = View.VISIBLE
                } else {
                    val imageInfo = chatMessageInfo.postObj?.images?.get(0)
                    Glide.with(context)
                        .load(if(!(imageInfo?.thumbnailUrl?.isNullOrEmpty() ?: false)) imageInfo?.thumbnailUrl else imageInfo?.image)
                        .placeholder(colorDrawable)
                        .centerCrop()
                        .into(ivMediaView)

                    postTypeLinearLayout.visibility = View.VISIBLE
                    reelTypeAppCompatImageView.visibility = View.GONE

                    Glide.with(context)
                        .load(chatMessageInfo.postObj?.user?.avatar)
                        .placeholder(colorDrawable)
                        .centerCrop()
                        .into(ivUserProfile)

                    tvUsername.text = chatMessageInfo.postObj?.user?.name ?: ""
                    ivVerified.isVisible = (chatMessageInfo.postObj?.user?.profileVerified ?: 0) == 1

                    tvLocation.text = chatMessageInfo.postObj?.user?.username ?: ""
                }
