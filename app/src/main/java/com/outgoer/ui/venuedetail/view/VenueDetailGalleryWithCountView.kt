...
class VenueDetailGalleryWithCountView(context: Context) : ConstraintLayoutWithLifecycle(context) {
    ...
    private fun inflateUi() {
        ...
        binding.flPost.throttleClicks().subscribeAndObserveOnMainThread {
            venueDetailGalleryCountViewClickSubject.onNext(Unit)
        }.autoDispose()
    }

    fun bind(venueGalleryItem: VenueGalleryItem, galleryCount: Int?) {
        ...
        binding.apply {
            ...
            if (galleryCount != null && galleryCount != 0) {
                ...
                tvPostCount.text = (galleryCount - 4).toString().plus("+")
            }
        }
    }
}