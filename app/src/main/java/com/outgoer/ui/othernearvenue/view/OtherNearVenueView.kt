...
class OtherNearVenueView(context: Context) : ConstraintLayoutWithLifecycle(context) {
    ...
    private fun inflateUi() {
        ...
        binding.apply {
            ...
            llVenueInfo.throttleClicks().subscribeAndObserveOnMainThread {
                otherNearPlaceClickSubject.onNext(OtherNearPlaceClickState.OtherNearPlaceClick(venueMapInfo))
            }.autoDispose()

            ivFavourite.throttleClicks().subscribeAndObserveOnMainThread {
            ...
            ivDirection.throttleClicks().subscribeAndObserveOnMainThread {
    ...
    private fun updateFavouriteStatus() {
        binding.apply {
            ...
            if (venueFavouriteStatus != null) {
                if (venueFavouriteStatus == 1) {
                    binding.ivFavourite.setColorFilter(resources.getColor(R.color.purple, null))
                    binding.ivFavourite.setImageResource(R.drawable.ic_favourite_venue_active)
                } else {
                    binding.ivFavourite.setColorFilter(resources.getColor(R.color.md_white, null))
